...
class Daextam_Menu_Elements {
	 ...
	 * The constructor.
	 *
	 * @param Daextam_Shared $shared An instance of the shared class.
	...
	public function display_list_table(
		...
		?>
			...
			<?php if ( count( $results ) > 0 ) : ?>
				...
				<div class="daextam-crud-table">

					<!-- list of tables -->
					<table class="daextam-crud-table__daext-items">
						<thead>
						<tr>
							<th>
								<input type="checkbox" class="daextam-cb-select-all">
							</th>
							<?php
						...
						<?php foreach ( $results as $result ) : ?>
							<tr>
								<td><input type="checkbox"
											class="daextam-bulk-action-checkbox"
								...
								foreach ( $columns as $key => $column ) {
									...
									<td>
										...
										<?php if ( 0 === $key ) : ?>
											<a class="daextam-crud-table__item-name" href="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $page_slug ); ?>&edit_id=<?php echo esc_attr( $result[ $db_primary_key ] ); ?>">
												<?php echo esc_html( stripslashes( $result[ $column['db_field'] ] ) ); ?>
											</a>
											<div class="daextam-crud-table__row-actions">
											<div class="daextam-crud-table__row-actions-single-action">
											...
											<div class="daextam-crud-table__row-actions-single-action">
												<form method="POST"
													...
													<?php wp_nonce_field( 'daextam_clone_' . $this->menu_slug . '_' . intval( $result[ $db_primary_key ], 10 ), 'daextam_clone_' . $this->menu_slug . '_nonce' ); ?>
													<button type="submit" name="clone_id" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>">Duplicate</button>
												</form>
											</div>
											<div>&nbsp|&nbsp</div>
											<div class="daextam-crud-table__row-actions-single-action daextam-crud-table__row-actions-single-action-delete">
												<form method="POST"
														action="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $page_slug ); ?>"
				...
				<!-- Bulk Actions -->
				<div class="daextam-crud-table-controls">
					...
					<div class="daextam-crud-table-controls__pagination-container">
	...
	public function header_bar() {
		...
		<div class="daextam-header-bar">

			<div class="daextam-header-bar__left">
	...
	public function new_item() {
		...
		<div class="daextam-main-form">
			...
			<form id="form1" name="form1" method="POST"
					action="admin.php?page=daextam-<?php echo esc_attr( $this->slug_plural ); ?>"
					autocomplete="off">
	...
	public function edit_item( $edit_id ) {
		...
		<div class="daextam-main-form">
			...
			<form id="form1" name="form1" method="POST"
				...
				// Sanitize the field name.
				$db_table_name = $wpdb->prefix . 'daextam_' . $this->db_table;
	...
	public function input_field(
		...
		<div class="daextam-main-form__daext-form-field" valign="top" data-section-id="<?php echo esc_attr( $section_id ); ?>">
			<div><label for="title"><?php echo esc_html( $label ); ?><?php echo $required ? ' <span class="daextam-required">*</span>' : ''; ?></label></div>
			<div>
	...
	public function display_admin_toolbar() {
		...
		<div class="daextam-admin-toolbar">
			...
			<div class="daextam-admin-toolbar__right-section">
				...
				<img class="daextam-admin-toolbar__daext-logo" src="<?php echo esc_url( $this->shared->get( 'url' ) . 'admin/assets/img/daext-logo.svg' ); ?>" alt="DAEXT" />
				</a>
			</div>
		</div>
	...
	public function display_pro_features() {
		...
		if ( constant( 'DAEXTAM_EDITION' ) !== 'FREE' ) {
			return;
		}
		...
		<div class="daextam-admin-body">
			...
			<div class="daextam-pro-features">

				<div class="daextam-pro-features__wrapper">
					...
					<div class="daextam-pro-features__right">
	...
	public function handle_duplicate() {
		...
		if ( ! is_null( $data['clone_id'] ) ) {
			...
			// Nonce verification.
			check_admin_referer( 'daextam_clone_' . $this->menu_slug . '_' . $data['clone_id'], 'daextam_clone_' . $this->menu_slug . '_nonce' );
	...
	public function handle_delete() {
		...
		if ( ! is_null( $data['delete_id'] ) ) {
			...
			// Nonce verification.
			check_admin_referer( 'daextam_delete_' . $this->menu_slug . '_' . $data['delete_id'], 'daextam_clone_' . $this->menu_slug . '_nonce' );
	...
	public function handle_bulk_actions() {
		...
		if ( 'delete' === $bulk_action ) {
			...
			if ( ! is_null( $delete_id ) ) {
				...
				// Nonce verification.
				check_admin_referer( 'daextam_bulk_action_' . $this->menu_slug, 'daextam_bulk_action_' . $this->menu_slug . '_nonce' );
	...
	public function print_form_fields_from_array( $sections ) {
		...
		foreach ( $sections as $section ) {
			...
			echo '<div class="daextam-main-form__daext-form-section" data-id="' . esc_attr( $section['section_id'] ) . '">';
			...
			echo '<div class="daextam-main-form__daext-form-section-body" data-section-id="' . esc_attr( $section['section_id'] ) . '">';
	...
	public function display_crud_menu_search_form( $post_search_input ) {
		...
		<form action="admin.php" method="GET" id="items-filter">
			...
			<div class="daextam-crud-table-search-form">

				<div id="daext-search-form">

					<input type="hidden" name="page" value="daextam-<?php echo esc_attr( $this->slug_plural ); ?>">
					<input class="daextam-crud-table-search-form__post-search-input" type="text" name="s"
							value="<?php echo null !== $post_search_input ? esc_attr( stripslashes( $post_search_input ) ) : ''; ?>" autocomplete="off" maxlength="255">
					<input class="button daextam-admin-page-button" type="submit" value="Search <?php echo esc_attr( $this->label_plural ); ?>">
